...
*/
import {applyChain, createChannel,joinChannel,updateChannel, genchanneltxfile,downloadchannelconfigfile,downloadorgconfigfile,downloadConnectionProfile, listChain, releaseChain, editChain, listDBChain, recentBlocks, queryChannels,recentTransactions, queryChainCodes, recentTokenTransfer, queryByBlockId, queryByTransactionId} from '../services/chain'
...
export default {
  ...
  effects: {
    ...
    * listDBChain ({payload}, {call, put}) {
      ...
      let currentChainId = localStorage.getItem(`${window.apikey}-chainId`)
      const data = yield call(listDBChain,{currentChainId})
      if (data && data.success) {
        ...
        if (data.chains.length > 0){
          ...
          for (let i=0; i<data.chains[0].orgs.length; i++){
           ...
           // for (let peerkey in data.chains[0].template.network[data.chains[0].orgs[i]].peers){
              var peerinfo = {};
    ...
    *downloadConnectionProfile ({payload}, {call, put}) {
      const data = yield call(downloadConnectionProfile, payload)
      if (data && data.success) {
        message.success(intl.formatMessage(messages.success.commonoperate))
      } else {
        message.error(intl.formatMessage(messages.fail.commonoperate))
      ...
      const fileName=payload.org+'MSP.tar.gz';

      // var repstr = data.data;
      //
      // var blob = new Blob([repstr]);

      var repstr = new Buffer(data.data, 'base64');
    ...
    *downloadorgconfigfile ({payload}, {call, put}) {
      const data = yield call(downloadorgconfigfile, payload)
      if (data && data.success) {
        message.success(intl.formatMessage(messages.success.commonoperate))
      } else {
        message.error(intl.formatMessage(messages.fail.commonoperate))
      ...
      const fileName=payload.org+'.json';
    ...
    *joinChannel ({payload}, {call, put}) {
      ...
      if (data && data.success) {
        ...
        yield put({type: 'setChannelJoining', payload: {channelJoining: false}})
    ...
    *updateChannel ({payload}, {call, put}) {
      ...
      if (data && data.success) {
        ...
        yield put({type: 'setChannelUpdating', payload: {channelUpdating: false}})
