...
chainSchema.post('save', function(doc, next) {
  // const {type, serviceUrl, id, size, username, user_id} = doc;
  //
  // const templateFile = `/home/lijisai/cello/user-dashboard/src/config-template/${type}/${size}peers/config.json`
  // let org_numbers = [1]
  // let peer_end = 1
  // if (size > 1) {
  //   org_numbers = [1, 2]
  ...
  //       }
  //
  //       model.findOneAndUpdate({_id: doc.id}, {template,orgs:temports},{upsert: true, new:true}, function (err, doc) {
  //         if (err) { logger.error(err) }
  //         next()
  //       })
  //     })
...
// chainSchema.post('save', function (doc, next) {
//   const {type, initialized, user_id, stage} = doc;
//   if (stage === "apply"){
//      model.findOneAndUpdate({_id: doc.id}, {stage: "createchannel"}, {upsert: true}, function (err, doc) {
//        if (err) { logger.error(err) }
//      })
//      return
//   }
