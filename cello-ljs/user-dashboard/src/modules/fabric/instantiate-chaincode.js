...
function initializeWithChannel (template, channelName, channelpeerlist)  {
    helper.initializeWithChannel(template,channelName, channelpeerlist);
    ORGS = template.network.application;
}
...
var instantiateChaincode = function(channelName, chaincodeName, chaincodeVersion, functionName, args, username, org) {
	...
	return helper.getOrgAdmin(org).then((user) => {
		...
		return channel.sendInstantiateProposal(request, 80000);
	}, (err) => {
		...
		if (all_good) {
			...
			let data = fs.readFileSync('/opt/cello/fabric-1.0/' + ORGS[org].peers['peer1'][
				'tls_cacerts'
			]);
			eh.setPeerAddr(ORGS[org].peers['peer1']['events'], {
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': ORGS[org].peers['peer1']['server_hostname']
			});
			eh.connect();
